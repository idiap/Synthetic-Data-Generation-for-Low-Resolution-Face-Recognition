...
# see LICENSES/MIT.txt. Changes: native input_size threaded through the DALI and MXNet pipelines.
#
import numbers
import os
...
def get_dataloader(
    ...
    if dali:
        return dali_data_iter(
            ...
            seed=seed if seed is not None else -1, name=dali_name,
...
def dali_data_iter(
    ...
    def dali_random_resize(img, resize_size, image_size=input_size):
    ...
    size_resize = fn.random.uniform(range=(int(input_size * 0.5), int(input_size * 0.8)), dtype=types.FLOAT)
    ...
    pipe = Pipeline(
        ...
        device_id=local_rank, prefetch_queue_depth=prefetch_queue_depth, seed=seed)
    condition_flip = fn.random.coin_flip(probability=0.5)
    with pipe:
        ...
        if dali_aug:
            ...
            images = multiplexing(condition_resize, dali_random_resize(images, size_resize, image_size=input_size), images)
